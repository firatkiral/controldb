...
/* 
  controldb-continuum - a stripped down port of  c#.net program to test capabilities of control and javascript.  
...
var ControlDBContinuum = (function () {
  ...
  Actor.prototype.executeActivity = function (activity) {
    ...
    if (this.isLoanPayment) {
      ...
      activity.renderedBalancePrimary = fpr.postVolatile(principlePaid, activity.activityDate, this.name + " (Principle)", this.$ctrl);
      ...
      if (fsr != null) {
        ...
        fsr.postVolatile(-(this.triggerAmount - principlePaid), activity.activityDate, this.name + " (Overhead)", this.$ctrl);
        activity.renderedBalanceSecondary = fsr.postVolatile(-principlePaid, activity.activityDate, this.name + " (Principle)", this.$ctrl);
      }
    ...
    if (this.isCompoundInterestAccrual) {
      if (this.periodicity != ActorPeriodicityEnum.Monthly) {
        ...
        activity.renderedBalancePrimary = fpr.postVolatile(interestAccrued, activity.activityDate, this.name, this.$ctrl);
      }
    ...
    activity.renderedBalancePrimary = fpr.postVolatile(this.triggerAmount, activity.activityDate, this.name, this.$ctrl);
    ...
    if (this.fundSecondary != null) {
      ...
      activity.renderedBalanceSecondary = fsr.postVolatile(this.triggerAmount * -1, activity.activityDate, this.name, this.$ctrl);
    }
  };
...
function addTestData() {
  ...
  var actorMortgagePayment = continuum.addActor({
    ...
    fundPrimary: fundMortgage.$ctrl, 	// apply payment to
    fundSecondary: fundChecking.$ctrl, // feeder fund
    periodicity: continuum.enums.ActorPeriodicity.Monthly,
    periodicityUnits: 1,
    triggerDateInitial: (new Date()).getTime(),
    triggerAmount: 800,
  ...
  var actorCarPayment = continuum.addActor({
    name: 'auto loan',
    fundPrimary: fundAutoLoan.$ctrl,
    fundSecondary: fundChecking.$ctrl, // feeder fund is checking
    periodicity: continuum.enums.ActorPeriodicity.Monthly,
    periodicityUnits: 1,
    triggerDateInitial: (new Date()).getTime(),
...
function dbLoaderCallback() {
  ...
  setTimeout(function () {
    ...
    logActivities(continuum.activitiesByFund(checkingFund.$ctrl));
    console.log("");
...
var continuum = new ControlDBContinuum("ControlDBContinuum.db", {
  autoload: true,
